...
const NewProduct = () => {
    ...
    const deleteImagePreview = (index) => {
        setImagesPreview((oldPreviews) => oldPreviews.filter((_, i) => i !== index));
    ...
    return (
        <>
            <MetaData title="Admin: New Product | Newcart" />
    ...
    {imagesPreview.map((image, index) => (
        <div key={index} className="relative">
            <img src={image} alt={`Product ${index + 1}`}  draggable="true"  className="w-full h-full object-contain rounded" />
             <span
               className="absolute top-1 right-1 cursor-pointer bg-white rounded-full p-1 hover:bg-red-500 hover:text-white"
...
//     const handleProductImageChange = (e) => {
//         console.log("Event:", e); // Check the event object first
    
//         const files = Array.from(e.target.files);
//         console.log("Selected Files:", files); // Check if files are correctly selected
...
//             reader.onload = () => {
//                 if (reader.readyState === 2) {
//                     console.log("File loaded:", file.name); // Check if file is loaded correctly
//                     setImagesPreview((oldImages) => [...oldImages, reader.result]);
...
//                 console.error("Error reading file:", error); // Handle FileReader errors
//             };
